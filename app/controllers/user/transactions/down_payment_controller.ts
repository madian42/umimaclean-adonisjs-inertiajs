...
export default class DownPaymentController {
  ...
  async index({ inertia, params, response, session, auth }: HttpContext) {
    ...
    // Get latest transaction for this order and type down_payment
  ...
  async handle({ request, params, response, session, auth }: HttpContext) {
    ...
    // Guard: prevent creating a new payment if there is an active one
    const latestTx = await Transaction.query()
      .where('order_id', order.id)
      .andWhere('type', 'down_payment')
      .orderBy('created_at', 'desc')
      .first()
    ...
    if (latestTx) {
      ...
      if (isPending && !isExpired) {
        ...
        return response.redirect().toRoute('transactions.dp.show', { id: order.number })
      }
    ...
    const amount = this.calculateDpAmount(order)
    ...
    try {
      ...
      const actions = result.actions.find((a) => a.name === 'generate-qr-code-v2')
      const midtransQrCode = actions?.url.split('/')[6]
   ...
   * - 12km → 25,000
   */
  private calculateDpAmount(order: Order): number {
